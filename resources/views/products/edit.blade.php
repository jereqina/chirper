  ...
  <div class="min-h-full">
    ...
    <main class="flex justify-center py-3">
      <div class="w-full max-w-lg bg-white p-8 shadow-lg rounded-lg">
        <!-- Form Header -->
        <div class="mb-4">
          <h2 class="text-xl font-bold text-white bg-indigo-600 p-4 text-center rounded-md">Edit Product Form</h2>
        </div>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" class="space-y-6">
          ...
          <div class="form-group">
            ...
            <input type="text" id="productName" name="ProductName" value="{{ $product->ProductName }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 input-focus" required>
          </div>

          <!-- Unit Price -->
          <div class="form-group">
            <label for="unitPrice" class="block text-lg font-medium text-gray-700">Unit Price</label>
            <input type="number" id="unitPrice" name="UnitPrice" value="{{ $product->UnitPrice }}" step="0.01" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 input-focus" required>
          </div>

          <!-- Stock Quantity -->
          <div class="form-group">
            <label for="stockQuantity" class="block text-lg font-medium text-gray-700">Stock Quantity</label>
            <input type="number" id="stockQuantity" name="StockQuantity" value="{{ $product->StockQuantity }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 input-focus" required>
          </div>

          <!-- Action Buttons -->
          <div class="flex space-x-4">
            <button type="submit" class="w-full bg-indigo-600 text-white font-bold px-4 py-2 rounded-md hover:bg-indigo-500">Update Product</button>
